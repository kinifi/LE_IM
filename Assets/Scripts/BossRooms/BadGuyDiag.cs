...
public class BadGuyDiag : MonoBehaviour {
	...
	//Configs for patrol
	private Vector2 upLeft = Vector2.up + (-Vector2.right);
	private Vector2 downRight = -Vector2.up + Vector2.right;
	...
	private void Patrol ()
	{
		...
		{
			ZeroOutFources();
			rigidbody2D.AddForce(upLeft * speed * Time.deltaTime);
		}
		else
		{
			ZeroOutFources();
			rigidbody2D.AddForce(downRight * speed * Time.deltaTime);
		}
	}
	...
	//Get boost when player sighted & scream
	private void UpdateSightedSpeed ()
	{
		if(facingRight)
		{
			sightedSpeed = downRight * 1000.0f;
			audio.pitch = 1.5f;
	...
	//Method to allow robbe's movement again
